            ...
            try {
                ...
                if (loggerSettings.showTrace.get()) {
                    addErrorTrace(
                        ...
                        true);
                    if (pos != noPos)
                        addErrorTrace(e, pos, "while evaluating call site%s", "", true);
                }
                throw;
            }
...
void ExprOpConcatLists::eval(EvalState & state, Env & env, Value & v)
{
    ...
    Value v2; e2->eval(state, env, v2);
    Value * lists[2] = { &v1, &v2 };
    state.concatLists(v, 2, lists, pos, "while evaluating one of the elements to concatenate into a list");
}
...
void ExprConcatStrings::eval(EvalState & state, Env & env, Value & v)
{
    ...
    for (auto & [i_pos, i] : *es) {
        ...
        if (firstType == nInt) {
            ...
            if (!first) {
                auto part = state.coerceToString(i_pos, *vTmp, context, false, false, "while evaluating one of the elements to concatenate into a path");
                if (sSize <= 1 && !hasPrefix(*part, "/") && accessor != state.rootFS.get_ptr())
                    state.error(
                        ...
                        .atPos(i_pos).withFrame(env, *this).debugThrow<EvalError>();
                sSize += part->size();
                s.emplace_back(std::move(part));
            }
        } else {
            if (s.empty()) s.reserve(es->size());
            auto part = state.coerceToString(i_pos, *vTmp, context, false, firstType == nString, "while evaluating one of the elements to concatenate into a string");
            sSize += part->size();
            s.emplace_back(std::move(part));
        }
...
{
    return coerceToString(pos, v, context, false, true, errorCtx);
}
...
SourcePath EvalState::coerceToPath(const PosIdx pos, Value & v, PathSet & context, std::string_view errorCtx)
{
    ...
    if (v.type() == nString) {
        copyContext(v, context);
        return decodePath(v.str(), pos, errorCtx);
    }
    ...
    error("cannot coerce '%1%' to a path", showType(v)).withTrace(pos, errorCtx).debugThrow<EvalError>();
}
