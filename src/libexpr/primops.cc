...
static RegisterPrimOp primop_abort({
    ...
    {
        PathSet context;
        std::string_view errorCtx = "while evaluating the error message passed to builtins.abort";
        auto s = state.decodePaths(*state.coerceToString(pos, *args[0], context, errorCtx), pos, errorCtx);
...
static RegisterPrimOp primop_throw({
    ...
    {
      PathSet context;
      std::string_view errorCtx = "while evaluating the error message passed to builtin.throw";
      auto s = state.decodePaths(*state.coerceToString(pos, *args[0], context, errorCtx), pos, errorCtx);
...
static void prim_addErrorContext(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    try {
        ...
        PathSet context;
        std::string_view errorCtx = "while evaluating the error message passed to builtins.addErrorContext";
        e.addTrace(nullptr, state.decodePaths(*state.coerceToString(pos, *args[0], context, errorCtx), pos, errorCtx));
        throw;
    }
}
...
static void prim_trace(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    if (args[0]->type() == nString)
        printError("trace: %1%", state.decodePaths(args[0]->string.s, pos, "while evaluating the first argument passed to builtins.trace"));
    else
            ...
            /* The `args' attribute is special: it supplies the
               command-line arguments to the builder. */
                ...
                for (auto elem : i->value->listItems()) {
                    auto s = state.coerceToString(posDrvName, *elem, context, true, true,
            ...
            /* All other attributes are passed to the builder through
               the environment. */
                ...
                if (jsonObject) {
                    ...
                    auto s = state.coerceToString(i->pos, *i->value, context, true, true, "while evaluating an attribute passed to builtins.derivationStrict").toOwned();
...
{
    PathSet context;
    auto path = state.coerceToPath(pos, *args[1], context, "while evaluating the second argument (the path to filter) passed to builtins.filterSource");
    state.forceFunction(*args[0], pos, "while evaluating the first argument (the filter function) passed to builtins.filterSource");
