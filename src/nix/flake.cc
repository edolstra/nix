            ...
            try {
                ...
                if (auto attr = v.attrs->get(state->symbols.create("path"))) {
                    if (attr->name == state->symbols.create("path")) {
                        PathSet context;
                        auto path = state->coerceToPath(attr->pos, *attr->value, context, "while evaluating the `path` attribute of a flake template");
                        if (!path.pathExists())
...
struct CmdFlakeInitCommon : virtual Args, EvalCommand
{
    ...
    void run(nix::ref<nix::Store> store) override
    {
        ...
        PathSet context;
        auto templateDir = evalState->coerceToPath(noPos, templateDirAttr, context, "while evaluating the `path` attribute of a flake template installable");
